...
class Pop(object):
    ...
    def __init__(self, R=[],A=[], Cost1=0,Cost2=0) :
        self.R = R
        self.A = A
        self.Cost1 = Cost1
        self.Cost2 = Cost2
...
def POFsort(p):
    def Return_Costs(elem):
        return round(elem.Cost2,1),round(elem.Cost1,1)
    p.sort(key=Return_Costs)
    return p
...
if __name__ == "__main__":
    ...
    if r==0:
        ...
        ypool.join()
        for i in range(nPop0):
            p.append(Pop(x[i],y[i],result[i][0],result[i][1]))
    ...
    while r<(MaxIt+1):
        ...
        for j in range(g):    
            #print Z[j]
            p1=[]
            p1.append(Pop(X[j],Y[j],result[j][0],result[j][1]))               
        ...
        BestCosts.append(BestSol.Cost1)
